...
var Tasas = require('../models/tasas');
	   			...
	   			if (err){
	   				...
	   				if(!itemsFound){
						res.status(404).send({message: 'Imposible mostrar información'});
		   				...
		   				Tasas.countDocuments({}, (err,conteo) =>{
		   					res.status(200).send({
								items: itemsFound,
								total: conteo
							});
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Tasas.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
