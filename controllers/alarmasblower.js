...
function registraItem(req,res){
	...
	var item = new Alarmasblower(req.body);
...
function itemsRangoUltimos(req,res){
	...
	var idblower = req.query.idblower; 
	items = Number(items);
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Alarmasblower.find({'blower': idblower})
	   .skip(0)
	   .limit(items)
	   			...
	   			if (err){
	   				...
	   				if(!itemsFound){
						res.status(404).send({message: 'Imposible mostrar información'});
		   				...
		   				Alarmasblower.countDocuments({}, (err,conteo) =>{
		   					res.status(200).send({
								items: itemsFound,
								total: conteo
							});
...
function itemsRangoFechas(req,res){
	...
	var idblower = req.query.idblower;
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Alarmasblower.find({'blower': idblower, ts : {
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Alarmasblower.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
