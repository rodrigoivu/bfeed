...
function registraItem(req,res){
	...
	var item = new Alarmasselector(req.body);
...
function itemsRangoUltimos(req,res){
	...
	var idselector = req.query.idselector; 
	items = Number(items);
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Alarmasselector.find({'selector': idselector})
	   .skip(0)
	   .limit(items)
	   			...
	   			if (err){
	   				...
	   				if(!itemsFound){
						res.status(404).send({message: 'Imposible mostrar información'});
		   				...
		   				Alarmasselector.countDocuments({}, (err,conteo) =>{
		   					res.status(200).send({
								items: itemsFound,
								total: conteo
							});
...
function itemsRangoFechas(req,res){
	...
	var idselector = req.query.idselector;
	//OJO CAMBIAR NOMBRE DE COLLECCION Y CAMPOS SEGÚN LA CONSULTA
	Alarmasselector.find({'selector': idselector, ts : {
...
function deleteItem(req,res){
	...
	//OJO CAMBIAR NOMBRE DE COLLECCION SEGÚN LA CONSULTA
	Alarmasselector.findByIdAndRemove(itemId, (err, itemRemoved) => {
		if(err){
